...
class GeoCentric{
    ...
    constructor(parent:HTMLElement){
        ...
        this.center = new Ball(new Vector(this.canvas.width/2,this.canvas.height/2))
    ...
    private animate = ()=>{
        ...
        for(var i=0;i<this.balls.length;i++){
            for(var j=0;j<this.balls.length;j++){
                if(i!=j){
                    const nVec = new Vector(this.balls[j].pos.x - this.balls[i].pos.x, this.balls[j].pos.y - this.balls[i].pos.y)
